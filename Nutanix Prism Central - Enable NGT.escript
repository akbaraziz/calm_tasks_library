...
cluster_uuid_url = "https://" + pc_ip + \
    ":9440/api/nutanix/v3/clusters/" + cluster_uuid
headers = {
...
method = 'POST'
url = "https://" + cluster_ip + ":9440/PrismGateway/services/rest/v1/vms/" + \
    vm_uuid + "/guest_tools/mount"
print("Making a {} API call to {}".format(method, url))
resp = process_request(url, method, pc_user, pc_password, headers)
if resp.content:
    result = json.loads(resp.content)
...
method = 'POST'
url = "https://" + cluster_ip + ":9440/PrismGateway/services/rest/v1/vms/" + \
    cluster_uuid + "::" + vm_uuid + "/guest_tools/"
print("Making a {} API call to {}".format(method, url))
payload = {
